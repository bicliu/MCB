...
* Return         : 读取到的字符
*******************************************************************************/
static USART_TypeDef* COM_USART[2] = {USART1,USART2}; 
int fgetc(FILE *f)
	{
	/* Loop until received a char */
	while(!(USART_GetFlagStatus(COM_USART[USED_COM_NUMBER], USART_FLAG_RXNE) == SET))
		{
		}
	
	/* Read a character from the USART and RETURN */
	return (USART_ReceiveData(COM_USART[USED_COM_NUMBER])); //clay for com2
	}
